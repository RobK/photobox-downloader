...
module.exports = function (logger){
  ...
  return {
    ...
    downloadPhoto : function downloadPhoto(options, callback, attempts) {
      attempts = attempts || 0;
          ...
          if (!err && response.statusCode === 200) {
            ...
            if (exists === true && options.skipExisting === true ) {
              ...
              try {
                ...
                attempts++;
                logger.debug('  Error while downloading photo (attempt', attempts, 'of', MAX_ATTEPMTS, ')');
                logger.debug('  Reported Error: ', e);
                setTimeout(function () {
                  self.downloadPhoto(options, callback, attempts)
                }, (2000*(attempts * attempts)));
              }
            }
          } else {
        ...
        if (err) {
          ...
          async.eachSeries(
            ...
            function (body, callbackFn) {
              logger.debug('Extracting photos new ablbum page');
              options.body = body;
