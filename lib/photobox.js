...
var request;
var config;
var debug = {photoUrls: [], photoDownloadUrls: []};
...
module.exports = {
  ...
  getAlbumList : function getAlbumList() {
    ...
    debug.albumObjects = JSON.stringify(albumObjects);
    ...
    request(photoUrl,
      function (err, response, body) {
        if (!err && response.statusCode === 200) {
          ...
          // Save photo
          debug.photoDownloadUrls.push($('img').attr('src'));
          request($('img').attr('src'), callback).pipe(fs.createWriteStream(path.join(options.outputDir, name)));
        } else {
          callback(err);
        }
      }
